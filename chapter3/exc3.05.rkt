...
; and false for points not in the region. For example, the region contained
; within a circle of radius 3 centered at (5, 7) is described by the predicate
; that tests whether (x−5)2+(y−7)2≤32. To estimate the area of the region
; described by such a predicate, begin by choosing a rectangle that contains
; the region. For example, a rectangle with diagonally opposite corners at (2,
; 4) and (8, 10) contains the circle above. The desired integral is the area of
; that portion of the rectangle that lies in the region. We can estimate the
...
; region. If we try this with many points, then the fraction of points that
...
; this in terms of the random procedure used in 1.2.6, which returns a
; nonnegative number less than its input.136
...
; Monte-carlo will return the fraction of points that satisfy P, so to estimate
; the integral we multiply it by the area of the encompassing rectangle.
